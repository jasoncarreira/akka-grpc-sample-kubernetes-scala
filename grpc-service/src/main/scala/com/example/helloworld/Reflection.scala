...
object Reflection {
  ...
  final val AdditionalDescriptors =
    List(AnnotationsProto.javaDescriptor, ReflectionProto.javaDescriptor)
      .flatMap(flattenDependencies)
      .distinct

  private def flattenDependencies(
      descriptor: FileDescriptor): List[FileDescriptor] = {
    descriptor :: descriptor.getDependencies.asScala.toList
      .flatMap(flattenDependencies)
  }

  def serve(fileDescriptors: Seq[FileDescriptor], serviceNames: Seq[String])(
      implicit mat: Materializer,
      sys: ActorSystem): PartialFunction[HttpRequest, Future[HttpResponse]] = {
    implicit val ec: ExecutionContext = mat.executionContext

    import ServerReflection.Serializers._

    val handler = handle((AdditionalDescriptors ++ fileDescriptors)
                           .map(fd => fd.getName -> fd)
                           .toMap,
                         serviceNames)
  ...
  private final def containsSymbol(symbol: String,
                                   fileDesc: FileDescriptor): Boolean =
    (symbol.startsWith(fileDesc.getPackage)) && // Ensure package match first
      (splitNext(
        if (fileDesc.getPackage.isEmpty) symbol
        else symbol.drop(fileDesc.getPackage.length + 1)) match {
        case ("", "")            => false
        case (typeOrService, "") =>
        ...
        case (service, method) =>
          Option(fileDesc.findServiceByName(service))
            .exists(_.findMethodByName(method) != null)
      })

  private final def splitNext(name: String): (String, String) = {
  ...
  private final def findFileDescForSymbol(
      symbol: String,
  ...
  private final def containsExtension(container: String,
                                      number: Int,
                                      fileDesc: FileDescriptor): Boolean =
    fileDesc.getExtensions.iterator.asScala.exists(ext =>
      container == ext.getContainingType.getFullName && number == ext.getNumber)

  private final def findFileDescForExtension(
      container: String,
      number: Int,
      fileDescriptors: Map[String, FileDescriptor]): Option[FileDescriptor] =
    fileDescriptors.values.collectFirst {
      case fileDesc if containsExtension(container, number, fileDesc) =>
        fileDesc
    }

  private final def findExtensionNumbersForContainingType(
      container: String,
  ...
  private def handle(fileDescriptors: Map[String, FileDescriptor],
                     services: Seq[String])
    : Flow[ServerReflectionRequest, ServerReflectionResponse, NotUsed] =
    Flow[ServerReflectionRequest].map(req => {
      ...
      val response = req.messageRequest match {
        ...
        case In.FileByFilename(fileName) =>
          val list =
            fileDescriptors.get(fileName).map(_.toProto.toByteString).toList
          Out.FileDescriptorResponse(FileDescriptorResponse(list))
        case In.FileContainingSymbol(symbol) =>
          val list = findFileDescForSymbol(symbol, fileDescriptors)
        ...
        case In.FileContainingExtension(ExtensionRequest(container, number, unknown)) =>
          val list = findFileDescForExtension(
            container,
            number,
            fileDescriptors).map(_.toProto.toByteString).toList
          Out.FileDescriptorResponse(FileDescriptorResponse(list))
        case In.AllExtensionNumbersOfType(container) =>
          val list = findExtensionNumbersForContainingType(
            container,
            fileDescriptors) // TODO should we throw a NOT_FOUND if we don't know the container type at all?
          Out.AllExtensionNumbersResponse(
            ExtensionNumberResponse(container, list))
